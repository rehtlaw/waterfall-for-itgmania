...
local LeaderboardRequestProcessor = function(res, master)
	...
	if data and data[playerStr] then
		if HashCacheEntry(GetSongAndSteps(player)) ~= data[playerStr]["chartHash"] then
			return
		end
				...
				if data[playerStr]["itl"]["itlLeaderboard"] then
					for entry in ivalues(data[playerStr]["itl"]["itlLeaderboard"]) do
						if entry["isSelf"] then
							UpdateItlExScore(player, HashCacheEntry(GetSongAndSteps(player)), entry["score"])
...
local af = Def.ActorFrame({
  ...
  SetCommand = function(self)
  	chartchanged = true
  	self:stoptweening():sleep(0.5):queuecommand("ChartParsed")
  end,
	...
	RequestResponseActor(0, 0) .. {
		...
		MakeRequestCommand = function(self)
			...
			if SL[pn].ApiKey ~= "" and HashCacheEntry(GetSongAndSteps(player)) then
				query["chartHashP" .. n] = HashCacheEntry(GetSongAndSteps(player))
			...
			if sendRequest then
				...
				-- 	UpdatePathMap(player, HashCacheEntry(GetSongAndSteps(player)))
				-- end
